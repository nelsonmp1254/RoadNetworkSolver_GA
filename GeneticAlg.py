...
def contains(path, node = grid[1][7]):
    for x in path:
        if x == node:
            return True
    return False

def cleanup(p1, midNode = grid[1][7], endNode = grid[9][9]):
    i = 0
    while i < len(p1):
...
def breed(p1, p2, midNode = grid[1][7], endNode = grid[9][9]):
    child1 = list()
    child2 = list()
    parent1 = p1.route
    parent2 = p2.route
...
def mutate(p1, mutateFactor=0.1, midNode = grid[1][7]):
...
def main():
    ...
    midNode   = grid[1][7]  # mid city here
    ...
    for i in range(startPop):
        lastNode   = startNode
        startNode  = grid[0][0]    # fist city here
        midNode    = grid[1][7]    # middle city here
    ...
    for i in range(len(pop)):
        pop[i] = Path(cleanup(pop[i].route))
    ...
    plt.legend(['Min Cost', 'Average Cost'], loc='upper left')
    plt.show()
